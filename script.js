...
const game = {
    ...
    stop() {
        ...
        /* или проще это оставить? score.innerText = 'GAME OVER';*/
        var gameStop = document.getElementById('game');
        var gameOver = document.createElement('div');
        gameOver.className = "score";
        gameOver.innerHTML = 'GAME OVER';
        gameStop.insertBefore(gameOver, gameStop.children[0]);
	
	/* Отключаем все кнопки после поражения*/
...
//	startButton.addEventListener('click', game.start.bind(game));
    ...
    move(event) {
        ...
        /* смотрим статус игры*/
        if (game.getElement().getAttribute('class') == 'game stopped'){
            return;
	} else if (game.getElement().getAttribute('class') == 'game paused'){
            return;
	}
        ...
        /* в новой координате ищем пересечение с собой */
        ...
        if (foundFood !== -1) {
            ...
            /* считаем съеденную еду и выводим количество*/
            countScore ++;
            score.innerText = countScore;
...
const food = {
	...
	/* ищем пересечение координат новой еды с координатами змейки, если есть - генерируем новые координаты*/
	for (el in snake.parts){
		if (newItem.top === snake.parts[el].top && newItem.left === snake.parts[el].left){
			food.generateItem();
			return;
		} 
	}

	/* ищем пересечение координат новой еды с едой на поле, если есть - генерируем новые координаты*/
	for (el in food.items){
		if (newItem.top === food.items[el].top && newItem.left === food.items[el].left){
			food.generateItem();
			return;
		} 
	}
...
function init() {
    ...
    continueButton.addEventListener('click', game.continue.bind(game));
